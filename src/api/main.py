...
from src.services.cat_intelligence_service import get_cat_intelligence_service
...
def run_analysis_background(user_id: str, all_songs: List[Dict], history_songs: List[Dict], mood: Optional[str] = None):
    ...
    try:
        ...
        # Get recommendations with mood filter
...
@app.get("/api/v1/chat/mood-recommendations")
async def get_mood_recommendations(
    ...
    Args:
        user_id: User identifier
        mood: Mood to get recommendations for
        limit: Number of recommendations
    """
    try:
        song_search_service = get_song_search_service()
        recommendation_service = get_recommendation_service()
        ...
        mood_queries = {
            ...
            "energetic": "energetic workout motivational pump",
            "tired": "gentle soothing lullaby soft",
            "anxious": "calming peaceful meditation zen",
            "romantic": "romantic love tender emotional",
            "nostalgic": "nostalgic classic memories old",
            "focused": "instrumental ambient focus study"
        }
        
        query = mood_queries.get(mood.lower(), mood)
        search_results = song_search_service.search_songs(query, limit=limit * 2)
        
        # Get user's taste profile
        profile = recommendation_service.get_taste_profile(user_id)
        
        # Use chatbot service to integrate taste profile with mood recommendations
        chatbot_service = get_chatbot_service()
        personalized_songs = chatbot_service.get_mood_recommendations_with_taste(
            user_id=user_id,
            mood=mood,
            taste_profile=profile,
            song_candidates=search_results,
            limit=limit
        )
        
        # Format response
        formatted_songs = []
        for song in personalized_songs:
            formatted_songs.append({
                ...
                "youtube_video_id": None,  # Will be fetched on play
                "taste_score": song.get("taste_score", None),  # Include taste score
                "mood_score": song.get("mood_score", None),  # Include mood score
                "final_score": song.get("final_score", None),  # Include final combined score
                "personalized": profile is not None  # Indicate if personalized
            })
        ...
        return {
            ...
            "taste_profile_used": profile is not None
        }
    except Exception as e:
        import logging
        logging.error(f"Mood recommendations error: {e}")
        return {
            "mood": mood,
            "songs": [],
            "count": 0,
...
@app.get("/api/v1/chat/mood-trend")
async def get_mood_trend(user_id: str):
    """Get user's mood trends over time"""
    try:
        chatbot_service = get_chatbot_service()
        trend = chatbot_service.get_user_mood_trend(user_id)
        return {
            "user_id": user_id,
            "trend": trend
        }
        ...
        import logging
        logging.error(f"Mood trend error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
@app.get("/api/v1/cat/analyze-song")
async def cat_analyze_song(
    ...
    """
    Get cat's intelligent reaction to a song using LLM.
    
    Returns cat's emotion, comment, and reaction.
    """
    try:
        cat_service = get_cat_intelligence_service()
        
        artist_list = [a.strip() for a in artists.split(',') if a.strip()]
        genre_list = [g.strip() for g in genre.split(',')] if genre else []
        
        reaction = cat_service.analyze_song_for_cat(title, artist_list, genre_list)
        ...
        import logging
        logging.error(f"Error in cat song analysis: {e}")
        raise HTTPException(status_code=500, detail=str(e))

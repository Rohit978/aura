...
class RecommendationService:
    ...
    def get_recommendations(
        ...
        # If mood is provided, search for mood-based songs
        if mood and len(all_candidates) < limit * 2:
            mood_queries = {
                ...
                "tired": "gentle soothing soft",
                "anxious": "calming peaceful zen",
                "romantic": "romantic love ballad",
                "nostalgic": "nostalgic classic retro",
                "focused": "instrumental ambient focus"
            }
            
            query = mood_queries.get(mood.lower(), mood)
            try:
                results = self.song_search_service.search_songs(query, limit=15)
                for song in results:
                    title_key = (song['title'].lower(), song['artists'][0].lower() if song.get('artists') else '')
                ...
                print(f"Error searching by mood: {e}")
        ...
        # IMPROVEMENT 1: Use mood vector offset instead of keyword matching
        # Create mood-adjusted taste vector
        adjusted_taste_vector = taste_vector.copy()
        if mood:
            try:
                mood_vector = self.embedding_service.embed_mood(mood)
                # Apply mood offset: user_vector = taste_vector + 0.3 * mood_vector
                mood_weight = 0.3
                adjusted_taste_vector = taste_vector + mood_weight * mood_vector
                # Normalize to maintain unit vector
                norm = np.linalg.norm(adjusted_taste_vector)
                if norm > 0:
                    adjusted_taste_vector = adjusted_taste_vector / norm
            except Exception as e:
                logger.warning(f"Error creating mood vector: {e}, using base taste vector")
        ...
        for candidate in all_candidates:
            try:
                ...
                # IMPROVEMENT 1: Compute similarity to mood-adjusted taste vector
                similarity = self.embedding_service.cosine_similarity(adjusted_taste_vector, emb)
                
                # IMPROVEMENT 2: Weighted scoring instead of multiplicative boosts
                # Compute mood match score (semantic similarity to mood)
                mood_match = 0.0
                if mood:
                    try:
                        mood_vector = self.embedding_service.embed_mood(mood)
                        mood_match = self.embedding_service.cosine_similarity(mood_vector, emb)
                    except Exception:
                        mood_match = 0.0
                ...
                # IMPROVEMENT 2: Weighted combination (prevents genre dominance)
                # final_score = 0.7 * similarity + 0.2 * mood_match + 0.1 * genre_match
                final_score = (
                    0.7 * float(similarity) +
                    0.2 * float(mood_match) +
                    0.1 * float(genre_match)
                )
                
                # IMPROVEMENT 4: Light audio feature re-ranking
                audio_boost = self._compute_audio_feature_boost(candidate, mood)
                ...
                if mood and mood_match > 0.5:
                    explanation += f" • Perfect for {mood} mood"
